...
def eval_AP(model, model_format, annotation_lines, anchors, class_names, model_input_shape, eval_type, iou_threshold, conf_threshold, elim_grid_sense, v5_decode, save_result, class_filter=None, start_indx=-1, end_indx=-1):
    '''
    ...
    '''c=0
    for j,k in annotation_records.items():
        print(j,k)
        print(c)
        c=c+1
    c=0
    input()
    for j,k in gt_classes_records.items():
        print(j,k)
        print(c)
        c=c+1'''
    
    end = time.time()
    print("get records time cost: {:.6f}s".format(end - start))
    ...
    if os.path.isfile(res_name) and caching:
        ...
        start=time.time()
        pred_classes_records = get_prediction_class_records(model, model_format, item_slice, anchors, class_names, model_input_shape, conf_threshold, elim_grid_sense, v5_decode, save_result)
        end = time.time()
        print("get predictions time cost: {:.6f}s".format(end - start))
    ...
    if eval_type == 'VOC':
        start=time.time()
        AP, APs = compute_mAP_PascalVOC(item_slice, gt_classes_records, pred_classes_records, class_names, iou_threshold)

        if class_filter is not None:
            get_filter_class_mAP(APs, class_filter)
        end = time.time()
        print("compute mAP time cost: {:.6f}s".format(end - start))
    elif eval_type == 'COCO':
...
def main():
    ...
    parser.add_argument(
        ...
        help='number of threads for running evaluation, default=%(default)s', default=4)
    parser.add_argument(
    ...
    print("Initialization Elapsed time: {:.2f} seconds".format(elapsed_time))
    
    start = time.time()
    annotation_lines = get_dataset(args.annotation_file, shuffle=False)
    end = time.time()
    print("Annotation time cost: {:.6f}s".format(end - start))
    ...
    N = len(annotation_lines) // 1000
    num_threads = args.num_threads
    indx=args.thread_indx
    chunk_size = N // num_threads
    start_i = indx * chunk_size
    end_i = start_i + chunk_size
    if indx == num_threads - 1:
        end_i= N
    ...
    if num_threads > 1:
        ...
        for i in range(num_threads):
            ...
            if i == num_threads - 1:
                end_i= N
            #item_slice = itertools.islice(data.items(), start, end)
            thread = threading.Thread(target=eval_AP, args=(model, model_format, annotation_lines, anchors, class_names, model_input_shape, args.eval_type, args.iou_threshold, args.conf_threshold, args.elim_grid_sense, args.v5_decode, args.save_result, class_filter:=class_filter,start_indx:=start_i,end_indx:=end_i))
            threads.append(thread)
            thread.start()
        end2 = time.time()
        for thread in threads:
            thread.join()
        end3 = time.time()
        print("Evaluation time cost: {:.6f}s {:.6f}s".format(end2 - start2, end3-end2))
    else:'''
    start2 = time.time()
    ...
    end2 = time.time()
    print("Evaluation time cost: {:.6f}s".format(end2 - start2))
